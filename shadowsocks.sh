...
function pre_install(){
    ...
    read -p "(Default password: gofly666.com):" shadowsockspwd
    [ -z "$shadowsockspwd" ] && shadowsockspwd="gofly666.com"
    echo
    echo "---------------------------"
...
function download_files(){
    if [ "$OS" == 'CentOS' ]; then
        ...
        if ! wget --no-check-certificate https://github.com/davidleecn/shadowsocks/blob/master/shadowsocks -O /etc/init.d/shadowsocks; then
            echo "Failed to download shadowsocks chkconfig file!"
            exit 1
        fi
    else
        if ! wget --no-check-certificate https://github.com/davidleecn/shadowsocks/blob/master/shadowsocks-debian -O /etc/init.d/shadowsocks; then
            echo "Failed to download shadowsocks chkconfig file!"
            exit 1
        fi
    fi
